...
.PHONY: install up down build clean logs bash certs composer db db-test test wait-db wait-rabbitmq wait-meilisearch php-cs-fix rector phpstan qa

install: down certs up wait-db wait-rabbitmq wait-meilisearch composer db
...
wait-meilisearch:
	...
	@until docker exec $(PHP_CONTAINER) sh -c "nc -z meilisearch 7700"; do \
		sleep 2; \
	done
