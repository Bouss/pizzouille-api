...
describe('Create Ingredient API Endpoint', function (): void {
    ...
    it('creates an ingredient successfully', function (): void {
        ...
        $transport = self::getContainer()->get('messenger.transport.integration_events');

        expect($transport->getSent())->toHaveCount(1);

        $message = $transport->getSent()[0]->getMessage();

        expect($message)->toBeInstanceOf(IngredientCreated::class)
            ->and($message->ingredientId)->toBe((string) $ingredientId)
            ->and($message->code)->toBe('tomato')
            ->and($message->name)->toBe(['en' => 'Tomato', 'fr' => 'Tomate'])
            ->and($message->type)->toBe('vegetable')
            ->and($message->cost)->toBe(2.5)
            ->and($message->createdAt)->toBe($ingredientCreatedAt);
    });
});
